...
def monday():
    ...
    if instruction_input == 'yes':
        ...
        print("you selected no, please press enter to continue to just the mystery of today.")
        print(joyful_mysteries[0])
    else:
...
def run_command(day):
    ...
    return True  # Indicate that a valid day was selected

# Loop to keep asking for a valid day until the user provides one
while True:
    user_input = input("Enter a day of the week: ").strip()  # Remove any extra spaces
    if run_command(user_input):
        break  # Exit the loop once a valid day is selected
